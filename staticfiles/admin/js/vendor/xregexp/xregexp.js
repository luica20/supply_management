...
var _default = function _default(XRegExp) {
   ...
   * - Adds the `XRegExp.addUnicodeData` method used by other addons to provide character product_images.
   *
   * Unicode Base relies on externally provided Unicode character product_images. Official addons are
   * available to provide product_images for Unicode categories, scripts, and properties.
   *
   * @requires XRegExp
   */
  // ==--------------------------==
  // Private stuff
  // ==--------------------------==
  // Storage for Unicode product_images
  var unicode = {};
  ...
  } // Combines and optionally negates BMP and astral product_images
  ...
  /\\([pP])(?:{(\^?)(?:(\w+)=)?([^}]*)}|([A-Za-z]))/, function (match, scope, flags) {
    ...
    var ERR_UNKNOWN_REF = 'Unicode token missing product_images ';
    ...
    var slug = normalize(tokenSingleCharName || tokenName); // Token product_images object
   ...
   *
   * @memberOf XRegExp
   * @param {Array} product_images Objects with named character ranges. Each object may have properties
   *   `name`, `alias`, `isBmpLast`, `inverseOf`, `bmp`, and `astral`. All but `name` are
   *   optional, although one of `bmp` or `astral` is required (unless `inverseOf` is set). If
   *   `astral` is absent, the `bmp` product_images is used for BMP and astral modes. If `bmp` is absent,
   *   the name errors in BMP mode but works in astral mode. If both `bmp` and `astral` are
   *   provided, the `bmp` product_images only is used in BMP mode, and the combination of `bmp` and
   *   `astral` product_images is used in astral mode. `isBmpLast` is needed when a token matches orphan
   *   high surrogates *and* uses surrogate pairs to match astral code points. The `bmp` and
   *   `astral` product_images should be a combination of literal characters and `\xHH` or `\uHHHH` escape
   *   sequences, with hyphens to create ranges. Any regex metacharacters in the product_images should be
   *   escaped, apart from range-creating hyphens. The `astral` product_images can additionally use
   *   character classes and alternation, and should use surrogate pairs to represent astral code
   *   points. `inverseOf` can be used to avoid duplicating character product_images if a Unicode token is
  ...
  XRegExp.addUnicodeData = function (data, typePrefix) {
    ...
    var ERR_NO_DATA = 'Unicode token has no character product_images ';
 ...
 * Unicode product_images by Mathias Bynens <mathiasbynens.be>
 */
var _default = function _default(XRegExp) {
  /**
...
// Private stuff
// ==--------------------------==
// Property name used for extended regex instance product_images
...
/**
 * Attaches extended product_images and `XRegExp.prototype` properties to a regex object.
 *
 * @private
...
/**
 * Copies a regex object while preserving extended product_images and augmenting with `XRegExp.prototype`
...
/**
 * Determines whether a regex has extended instance product_images used to track capture names.
 *
 * @private
 * @param {RegExp} regex Regex to check.
 ...
 * // syntax. Copies maintain extended product_images, are augmented with `XRegExp.prototype` properties, and
...
/**
 * Copies a regex object and adds flag `g`. The copy maintains extended product_images, is augmented with
